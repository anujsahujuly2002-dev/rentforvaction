...
$(function () {
    table = $("#subcity").DataTable({
        ...
        bLengthChange: false,
        ajax: {
            url: site_url + "/admin/destination/subcity",
        },
        dataType: "html",
        columns: [
            {
    ...
    $("#subcity").on("error.dt", function (e, settings, techNote, message) {
...
$("#sub-city-create").on("submit", function (e) {
    e.preventDefault();
    showLoader();
    $.ajax({
        url: site_url + "/admin/destination/subcity/store",
        type: "POST",
        ...
        error: (xhr, ajaxOptions, thrownError) => {
            ...
            if (xhr.status == "422") {
                ...
                $(this).find(".sub_city_name-error").html("");
                ...
                $(this)
                    .find(".sub_city_name-error")
                    .append(
                        `<span class="text-danger">${error.sub_city_name}</span>`
                    );
            }
            if (xhr.status == "401") {
                console;
...
$("#sub-city-edit").on("submit", function (e) {
    e.preventDefault();
    showLoader();
    $.ajax({
        url: site_url + "/admin/destination/subcity/update",
        type: "POST",
        data: new FormData(this),
        contentType: false,
        cache: false,
...
function deleteSubCity(id) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            showLoader();
            $.ajax({
                url: site_url + "/admin/destination/subcity/delete",
                type: "POST",
                dataType: "json",
                data: { id: id },
                cache: false,
