...
$(function () {
    table = $("#state").DataTable({
        ...
        bLengthChange: false,
        ajax: {
            url: site_url + "/admin/destination/state",
        },
        dataType: "html",
        columns: [
            {
    ...
    $("#state").on("error.dt", function (e, settings, techNote, message) {
...
$("#state-create").on("submit", function (e) {
    e.preventDefault();
    showLoader();
    $.ajax({
        url: site_url + "/admin/destination/state/store",
        type: "POST",
...
$("#state-edit").on("submit", function (e) {
    e.preventDefault();
    showLoader();
    $.ajax({
        url: site_url + "/admin/destination/state/update",
        type: "POST",
        data: new FormData(this),
        contentType: false,
        cache: false,
...
function deleteState(id) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            showLoader();
            $.ajax({
                url: site_url + "/admin/destination/state/delete",
                type: "POST",
                dataType: "json",
                data: { id: id },
                cache: false,
